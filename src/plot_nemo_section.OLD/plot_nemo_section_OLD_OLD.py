...
def plot_nemo_section(filename,field,overplot_field=None,title=None,
...
# HARDWIRED FOR ORCA025 FOR NOW!!
    ...
    coordsfile='/data/cr1/frsy/UKGO/ORCA025L75/coordinates3D_orca025l75_surface-mask.nc'
    maskvar='glbmsk'

    mask=(general_tools.read_nc_var(maskfile,maskvar)*general_tools.read_nc_var(gridfile,'tmaskutil'))
    e1t = general_tools.read_nc_var(coordsfile,'e1t')
    e2t = general_tools.read_nc_var(coordsfile,'e2t')

    try:
        fld_read = iris.load_cube(filename,field)
    except iris.exceptions.ConstraintMismatchError:
#       if we can't match the standard name try matching the internal variable name     
        fld_read = iris.load_cube(filename,iris.Constraint(cube_func=lambda nemoname: nemoname.var_name == field) )

# remove leading time dimension if necessary
    if fld_read.ndim == 4:
        fld = fld_read[rec]
    else:
        fld = fld_read        
...
#    =============================================
    ...
    if logscale:
        tiny_factor=0.0001
        if mnfld == 0.0:
            mnfld = tiny_factor * mxfld
        if mxfld == 0.0:
            mxfld = tiny_factor * mnfld
        if mnfld < 0.0 and mxfld < 0.0:
            levs = -np.logspace(np.log10(-mnfld),np.log10(-mxfld),nlevs+1) 
        if mnfld > 0.0 and mxfld > 0.0:
            levs = np.logspace(np.log10(mnfld),np.log10(mxfld),nlevs+1) 
        if mnfld < 0.0 and mxfld > 0.0:
            nlevs_neg = int(abs(mnfld)*nlevs/(mxfld-mnfld))
            levs_neg = -np.logspace(np.log10(-mnfld),np.log10(-mnfld*tiny_factor),nlevs+1) 
            nlevs_pos = int(mxfld*nlevs/(mxfld-mnfld))
            levs_pos = np.logspace(np.log10(mxfld*tiny_factor),np.log10(mxfld),nlevs+1) 
...
#    my_cmap = matplotlib.colors.ListedColormap(colorlist, 'my_cmap')
    ...
    if lineplot:
        ...
        clev = plt.contour(xpoints,depth.points,fld_xsec, colors='black',levels=levs)
#        plt.clabel(clev,inline=False,manual=True)
    elif block:
    ...
    if overplot_field is not None:
        plt.contour(xpoints.data,depth.points,ofield_xsec.data, colors='k',levels=overplotlevs)
...
if __name__=="__main__":
    ...
    parser.add_argument("-C", "--lineplot", action="store_true",dest="lineplot",
                    help="plot line contours instead of colour-filled contours.")
    ...
    parser.add_argument("-f", "--mnfld", action="store",dest="mnfld",type=float,
                    help="minimum field value to plot")
    parser.add_argument("-F", "--mxfld", action="store",dest="mxfld",type=float,
    ...
    plot_nemo_section(args.filename,args.field, outfile=args.outfile, title=args.title, lat=args.lat, lon=args.lon, nlevs=args.nlevs,
    rec=args.rec, xmin=args.xmin, xmax=args.xmax, mnfld=args.mnfld, mxfld=args.mxfld, depthmax=args.depthmax, 
    method=args.method, logscale=args.logscale, reverseX=args.reverseX, kilometres=args.kilometres, toldeg=args.toldeg,
