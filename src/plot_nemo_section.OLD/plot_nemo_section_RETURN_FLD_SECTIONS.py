...
Nov 2018    1. Create the get_section routine so I can call it from other modules. 
...
def get_section(fld_list,lons,lats,lons_bounds=None,lats_bounds=None,toldeg=None,method=None,
    ...
    if index_i is not None:
        ...
        for fld in fld_list:
            fld_xsec_list.append(fld.data[:,xmin:xmax+1,index_i])
        if len(lats.shape) == 1:
        ...
        for fld in fld_list:
            fld_xsec_list.append(fld.data[:,index_j,xmin:xmax+1])
        if len(lons.shape) == 1:
        ...
        if method == 'gridline':
            ...
            if len(lons.shape) == 1:
                ...
                for fld in fld_list:
                     fld_xsec_list.append(fld.data[:,j0,i0:i1+1])
            ...
            for fld in fld_list:
                ...
                fld_xsec_list.append(fld_xsec[:,sort_indices].copy())
        elif method == 'pat':
            start_lat = lat
            start_lon = xmin
            end_lat = lat
        ...
        if method == 'gridline':
            ...
            if len(lons.shape) == 1:
                ...
                fld_xsec_list.append(fld.data[:,j0:j1+1,i0])
...
#            xpoints = np.squeeze(lons[sec_yind,sec_xind])
    ...
    return fld_xsec_list,xpoints,xpoints_bounds
...
#############################
    ...
    for fld,filename in zip(fld_list,filenames):
        ...
        # get_section takes a list of fields as an argument and returns a list of cross sections. 
        # here we send and receive lists of length 1.
        (fld_xsec, xpoints, xpoints_bounds) = get_section([fld],lons,lats,lons_bounds=lons_bounds,lats_bounds=lats_bounds, toldeg=toldeg,
                                                          method=method,lon=lon,lat=lat,index_i=index_i,index_j=index_j,xmin=xmin,xmax=xmax)
        fld_xsec_list.append(fld_xsec[0].copy())
        if maskzero:
