...
# ... or select required level
    ...
    if nscalar or not vec_only:
        ...
        for mnfld_i,mxfld_i,nlevs_i,fldslice_i,plot_type in zip(mnfld[:nscalar],mxfld[:nscalar],nlevs[:nscalar],fldslice[:nscalar],plot_types[:nscalar]):
            ...
            if cmap is not None and (plot_type == 'c' or plot_type == 'b'):
                if cmap == 'Rainbow':
...
#                ybounds.append(None)
    ...
    if 'c' in plot_types or 'b' in plot_types:
    ...
    if nscalar:
        for x_i,xbounds_i,y_i,ybounds_i,fldproj_i,plot_type,levs_i,fld_in_i in \
          zip(x[:nscalar],xbounds[:nscalar],y[:nscalar],ybounds[:nscalar],fldproj[:nscalar],plot_types[:nscalar],levs[:nscalar],fld_in[:nscalar]):
            ...
            if plot_type == 'l':
                ### contour lines ###
                cs1 = plt.contour(x_i,y_i,fldproj_i.data,levels=levs_i,mxfld=levs_i[-1],mnfld=levs_i[0],colorbar=None,colors=colors,linewidths=0.5)
            elif plot_type == 'c':
                ### colour-filled contours ###
                cs1 = plt.contourf(x_i,y_i,fldproj_i.data,levels=levs_i,mxfld=levs_i[-1],mnfld=levs_i[0],colorbar=None,cmap=cmap,transform=ccrs.PlateCarree())
            elif plot_type == 'b':
                ### block plot ###
                clev = plt.pcolormesh(xbounds_i, ybounds_i, fldproj_i.data, cmap=cmap, vmin=levs_i[0], vmax=levs_i[-1],transform=ccrs.PlateCarree())
            else:
                raise Exception("Error: Something has gone wrong. Scalar plot_type should be 'l' or 'c' or 'b'")

            ### colour bar ###
            if plot_type == 'c' or plot_type == 'b':
                cb=plt.colorbar(orientation='horizontal',cax=ax)
                cb.set_ticks(levs_i)
                if scientific:
                ...
                cb.ax.set_xlabel(str(fld_in_i.units))
