...
Nov 2018 : 1. Added block plotting option and rejigged control of plot types. DS.
           2. Got plotting of limited area data working (although still doesn't work
              for global plot extents).

To do    : 1. Cut out field if there are plot extent limits as an optimisation step.
...
def plot_nemo(filenames=None,sca_names=None,vec_names=None,nlevs=13,mnfld=None,mxfld=None,title=None,rec=0,level=1,bottom=False,scientific=False,
              cmap=None,colors=None,reverse_colors=False,glob=None,west=None,east=None,south=None,north=None,proj='pc0',maskfile=None,outfile=None,
...
# ... or select required level
    ...
    for lons_i,lats_i in zip(lons,lats):
        print 'min/max lons_i : ',lons_i.min(),lons_i.max() 
        print 'min/max lats_i : ',lats_i.min(),lats_i.max() 
    ...
    if region != 'global':
        for lons_i, lats_i, fldslice_i in zip(lons[:],lats[:],fldslice[:]): 
            shapedata = fldslice_i.data.shape
            if len(lons_i.shape) == 1:
                lons2D = lons_i * np.ones(shapedata)
            else:
                lons2D = lons_i
            if len(lats_i.shape) == 1:
                lats2D = lats_i * np.ones(shapedata)
            else:
                lats2D = lats_i
            fldslice_i.data = ma.masked_where( ((lons2D < region[0]) | (lons2D > region[1])) | ((lats2D < region[2]) | (lats2D > region[3])) , fldslice_i.data )
    ...
    if nscalar or not vec_only:
        ...
        for mnfld_i,mxfld_i,nlevs_i,fldslice_i,plot_type in zip(mnfld[:nscalar],mxfld[:nscalar],nlevs[:nscalar],fldslice[:nscalar],plot_types[:nscalar]):
            ...
            if logscale and ( plot_type == 'c' or plot_type == 'b' ):
                tiny_factor=0.0001
                if mnfld_i == 0.0:
...
#                cmap=cmap(ind)
    ...
    # it a workaround for a cartopy bug. 
          ...
          zip(x[:nscalar],xbounds[:nscalar],y[:nscalar],ybounds[:nscalar],fldproj[:nscalar],plot_types[:nscalar],levs[:nscalar],fld_in[:nscalar]):
            ...
            if plot_type == 'l':
                ...
                if colors is not None:
                    ...
                    cmap_block = cmap
                clev = plt.pcolormesh(xbounds_i, ybounds_i, fldproj_i.data, cmap=cmap_block, vmin=mnfld_i, vmax=mxfld_i)
            else:
                raise Exception("Error: Something has gone wrong. Scalar plot_type should be 'l' or 'c' or 'b'")

            ### colour bar ###
            if plot_type == 'c' or plot_type == 'b':
