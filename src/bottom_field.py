...
def bottom_field(infield=None,tmask=None,grid=None):
    ...
    if len(infield.shape) == 4:
        ...
        if grid == "W":
            ...
            # For U-grid fields need to use the T-mask to determine bottom points because U fields are set 
            # to zero rather than masked at normal edge points.
            for lev in range(len(infield[0,:,0,0])-1):
                bottom_level[:,:,:] = np.where( ( ( tmask[0,lev  ,:,:] & np.roll( tmask[0,lev  ,:,:],-1,1)) & \
                                                  (~tmask[0,lev+1,:,:] | np.roll(~tmask[0,lev+1,:,:],-1,1))), 
                                                  lev, bottom_level )
                bottom_field[:,:,:] = np.where( ( ( tmask[0,lev  ,:,:] & np.roll( tmask[0,lev  ,:,:],-1,1)) & \
                                                  (~tmask[0,lev+1,:,:] | np.roll(~tmask[0,lev+1,:,:],-1,1))), 
                                                  infield[:,lev,:,:], bottom_field )
        elif grid == "V":
            # For V-grid fields need to use the T-mask to determine bottom points because V fields are set
            # to zero rather than masked at normal edge points. 
            for lev in range(len(infield[0,:,0,0])-1):
                bottom_level[:,:,:] = np.where( ( ( tmask[0,lev  ,:,:] & np.roll( tmask[0,lev  ,:,:],-1,0)) & \
                                                  (~tmask[0,lev+1,:,:] | np.roll(~tmask[0,lev+1,:,:],-1,0))), 
                                                  lev, bottom_level )
                bottom_field[:,:,:] = np.where( ( ( tmask[0,lev  ,:,:] & np.roll( tmask[0,lev  ,:,:],-1,0)) & \
                                                  (~tmask[0,lev+1,:,:] | np.roll(~tmask[0,lev+1,:,:],-1,0))), 
                                                  infield[:,lev,:,:], bottom_field )
        else:
            for lev in range(len(infield[0,:,0,0])-1):
        ...
        if grid == "W":
            ...
            # For U-grid fields need to use the T-mask to determine bottom points
            ...
            # For V-grid fields need to use the T-mask to determine bottom points
            for lev in range(len(infield[:,0,0])-1):
                bottom_level[:,:] = np.where( ( ( tmask[0,lev  ,:,:] & np.roll( tmask[0,lev  ,:,:],-1,0)) & \
                                                (~tmask[0,lev+1,:,:] | np.roll(~tmask[0,lev+1,:,:],-1,0))), 
                                                  lev, bottom_level )
